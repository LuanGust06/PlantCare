...
API_KEY = "4jhhgl8fmw8ee7pMCaBMsEa8jFsYBzEaHzmiT3ns7qwhNb58Cd"    # Ask for your API key: https://web.plant.id/api-access-request/
...
def get_result(identification_id):
    ...
    while True:
        ...
        response = requests.post(endpoint + str(identification_id),
                                 json=params,
                                 headers=headers).json()
        sug = response["suggestions"]
        if sug is not None:
            for s in sug:
                details = s["plant_details"]
                return details["common_names"][0]
